...
public class ChessMatch {
	...
	public boolean[][] possibleMoves(ChessPosition sourcePosition){
		//imprime as posições possíveis a partir de uma posição de origem
		Position position = sourcePosition.toPosition();
		//converte a posição de xadrez para posição de matriz normal
	...
	public ChessPiece replacePromotedPiece(String type) {
		if(promoted == null) {
			throw new IllegalStateException("There is no piece to raplace.");
		}
	...
	private Piece makeMove(Position source, Position target) {
		...
		// #specialmove castling pro lado direito
		...
		// #specialmove castling pro lado esquerdo
		if(p instanceof King && target.getColumn() == source.getColumn() - 2) {
	...
	private boolean testCheckMate(Color color) {
		...
		List<Piece> list = piecesOnTheBoard.stream().filter(x -> ((ChessPiece)x).getColor() == color).collect(Collectors.toList());
			//captura numa lista todas as peças na color
		for (Piece p : list) {
			boolean [][] mat = p.possibleMoves();
			for (int i=0; i<board.getRows(); i++) {
				for (int j=0; j<board.getColumns(); j++) {
